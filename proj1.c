...
string_t *combined_input_string = NULL;
    ...
    for (;;)
    {
        ...
        case 0:
            if (c == '%')
            {
                if (prev == '\\')
                {
                    string_put_char(combined_input_string, '%');
                }
                else
                {
                    state = 1;
            ...
            {
                string_put_char(combined_input_string, c);
            }
            break;
...
void expand_file(char *input_filename, char *output_filename)
{
    ...
    combined_input_string->data = string_reverse(combined_input_string->data);

    expand_string(combined_input_string, output_filename);

    string_free(combined_input_string);
}
...
{
    combined_input_string = string_malloc(16);
